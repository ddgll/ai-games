function initGeneration (brain) {
  neat = new neataptic.Neat(
    ...
    null,
    {
      mutation: [neataptic.methods.mutation.MOD_BIAS, neataptic.methods.mutation.MOD_WEIGHT, neataptic.methods.mutation.MOD_ACTIVATION],
      popsize: POPULATION,
...
function calculateFitness () {
  ...
  if (bes) {
    ...
    bestSize = bes.roads.length
    bestNumGen = neat.generation
    bestBrain = JSON.stringify(bes.brain.toJSON())
    localStorage.setItem('air-bestsize', bes.roads.length)
