class Road {
  ...
  collide (x, y) {
    ...
    let collide = false
    if (ds < this.width / 4 - WALL_SIZE || df < this.width / 4 - WALL_SIZE && !this.contains()) {
      if (this.arc !== false) {
        console.log('ARC COLLIDE', collidePointArc(x, y, this.xCenter, this.yCenter, this.width / 2, this.rotation, HALF_PI))
        return collidePointArc(x, y, this.xCenter, this.yCenter, this.width / 2 + WALL_SIZE, this.rotation, HALF_PI)
      } else if (this.rect !== false) {
        console.log('RECT COLLIDE', collidePointArc(x, y, this.xCenter, this.yCenter, this.width / 2, this.rotation, HALF_PI))
        return collidePointRect(this.wall.x, this.wall.y, x, y, this.wall.w, this.wall.h)
      }
    }
    return false
  }
