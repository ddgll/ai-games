class Road {
  ...
  intersects (x, y) {
    if (this.arc) {
      return collidePointArc(x, y, this.xCenter, this.yCenter, this.length, this.rotation, HALF_PI)
    } else {
      return collidePointRect(x, y, this.xCenter - this.length / 2, this.yCenter - this.width / 2, this.length / 2, this.width)
    }
  }
  ...
  leftUp () {
    ...
    this.arc = true
    this.rotation = HALF_PI - QUARTER_PI
    this.xCenter = this.x
