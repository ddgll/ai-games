class Circuit {
  ...
  calculateFitness () {
    const last = this.getLastRoad()
    if (!last) return 1
    this.fitness = this.roads.length * 1000 / distance(this.x, this.y, last.x, last.y) + 1
    console.log('FF Calc', this.fitness, this.roads.length, distance(this.x, this.y, last.x, last.y))
    return this.fitness
  }
