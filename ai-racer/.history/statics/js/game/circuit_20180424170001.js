...
class Circuit {
  ...
  draw () {
    if (this.dead) return
    if (this.roads.find(r => r.type === 'upDown' || r.type === 'downUp' || r.type === 'leftRight' || r.type === 'rightLeft'))
    this.roads.forEach(road => road.draw())
    const last = this.getLastRoad()
    if (this.roads.length === 1 && last && last.type === 'leftDown') {
      console.log('LAST Type GOOOOOD !!!', r.type)
    }
  }
