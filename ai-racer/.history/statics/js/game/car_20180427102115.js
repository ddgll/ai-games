function Car (x, y, brain, isBest, isTest) {
  ...
  this.draw = function () {
    ...
    for (let i = 0, l = this.sensors.length, s, xs, ys; i < l; i++) {
      s = this.sensors[i]
      stroke(0, 255, 0)
      xs = 0 + this.seight * cos(s.angle)
      ys = 0 + this.seight * sin(s.angle)
      line(0, 0, xs, ys)
      if (this.obstacles[i]) {
        xs = this.obstacles[i] * cos(s.angle)
        ys = this.obstacles[i] * sin(s.angle)
        stroke(255, 0, 0)
        ellipse(xs, ys, 5, 5)
      }
    }
  ...
  this.sense = function (circuit) {
    ...
    for (let i = 0, l = this.sensors.length, s, x, y, next, obs; i < l; i++) {
      ...
      while (next) {
        ...
        if (!circuit.checkPoint(x, y)) obs = { x, y }
      }
      if (obs) {
        obstacles.push(distance(this.position.x, this.position.y, obs.x, obs.y))
      } else {
        obstacles.push(null)
      }
    }
  ...
  this.update = function (circuit) {
    ...
    this.obstacles.forEach(o => o ? o.add(this.velocity) : null)
