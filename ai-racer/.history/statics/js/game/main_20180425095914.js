...
function setup () {
  ...
  if (bestBrain) {
    try {
      let brain = saved ? neataptic.Network.fromJSON(JSON.parse(bestBrain)) : null
      if (brain) {
        best = new Circuit (100, 100, DEFAULT_CIRCUIT_SIZE, bestBrain, true)
...
function draw () {
  ...
  if (best && neat.generation) {
    log.elt.innerHTML += '<h1>Generation:' + neat.generation + '- best score:' + bestScore + ' best gen: ' + bestNumGen + '</h1><br> best fitness:' + bestFitness
    log.elt.innerHTML += 'Best path: ' + best.serialize()
  }
