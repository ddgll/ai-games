...
class Circuit {
  ...
  calculateFitness (maxscore, minscore) {
    const last = this.getLastRoad()
    if (!last) return 0
    this.normalizedScore = (this.score - minscore) / (maxscore - minscore)
    if (this.normalizedScore === Infinity) this.normalizedScore = 0
    this.fitness = 1 / distance(this.x, this.y, last.x, last.y)
    console.log('fff', this.normalizedScore, this.fitness, maxscore, minscore)
    // this.fitness += this.roads.length / DEFAULT_CIRCUIT_SIZE
