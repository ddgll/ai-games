...
function setup () {
  if (!bestSize) bestSize = DEFAULT_CIRCUIT_SIZE
...
function draw () {
  ...
  if (frameRate() < 10) slider.value(1)
  if (frameRate() < 15) sliderm.value(1)
  log.elt.innerHTML = ''
  if (best && neat.generation) {
    log.elt.innerHTML += '<h1>Gen:' + neat.generation + '- best score:' + Math.round(bestScore) + '- best size:' + bestSize + ' best gen: ' + bestNumGen + '</h1>'
    log.elt.innerHTML += 'best fitness:' + bestFitness
    log.elt.innerHTML += '<br>best path: ' + best.serialize()
    log.elt.innerHTML += '<small>x' + maxc + '</small>'
  }
