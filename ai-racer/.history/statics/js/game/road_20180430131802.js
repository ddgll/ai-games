class Road {
  ...
  collideLine(x, y, x2, y2, angle) {
    // console.log('collideLine', this.arc, this.rect, x, y, x2, y2, angle)
    let obs = false
    if (this.arc !== false) {
      let tmp = {}
      lineCircleCollide(x, y, x2, y2, this.xCenter, this.yCenter, this.wall.width, tmp)
      // console.log('lineCircleCollide', this.arc, this.rect, tmp, x, y, x2, y2)
      if (tmp) return tmp
    } else if (this.rect !== false) {
      for (let wall of this.wallLines) {
        obs = collideLineLine(x, y, x2, y2, wall.x, wall.y, wall.x + wall.w, wall.y + wall.h, true)
        console.log('collideLineLine', x, y, x2, y2, wall.x, wall.y, wall.x + wall.w, wall.y + wall.h)
