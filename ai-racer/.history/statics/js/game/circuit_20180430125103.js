...
class Circuit {
  ...
  checkLine(x1, y1, x2, y2, index, seight, angle) {
    console.log('checkLine', index, x1, y1, x2, y2, seight, angle, Math.ceil(seight / DEFAULT_ROAD_WIDTH))
    for (let i = index - 2, l = index + Math.ceil(seight / DEFAULT_ROAD_WIDTH), r, obs; i < l; i++) {
      if (i < 0) {
        r = this.roads[this.roads.length + i]
      } else {
        r = this.roads[i]
      }
      obs = r.collideLine(x1, y1, x2, y2, angle)
      if (obs) return obs
    }
    return false
  }
