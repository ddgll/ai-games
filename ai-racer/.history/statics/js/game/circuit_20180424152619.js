...
class Circuit {
  ...
  isClosed () {
    ...
    if (last.xFin === this.x && last.yFin === this.y) {
      return true
    }
    return false
  }
  ...
  calculateFitness () {
    ...
    this.fitness = distance(this.x, this.y, last.x, last.y) + 1 / dista
    this.fitness += this.score
    console.log('FF Calc', this.fitness, this.score, length * DEFAULT_ROAD_LENGTH, distance(this.x, this.y, last.x, last.y) + 1)
    return this.fitness
  }
  ...
  update () {
      ...
      if (this.isFake) {
        switch (this.roads.length) {
          case 0:
            ...
            break;
          case 1:
            this.roads.push(new Road(last.x, last.y, 'upRight'))
            break;
          case 2:
            this.roads.push(new Road(last.x, last.y, 'leftUp'))
            break;
          case 3:
    ...
    if (!this.dead) {
      ...
      if (last) {
        ...
        // this.score += distance(this.x, this.y, last.x, last.y) + 1 / dista
      }
      if (this.roads.length === this.size) {
