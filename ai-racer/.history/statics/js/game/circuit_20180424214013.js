...
class Circuit {
  ...
  getNextType (last_) {
    ...
    if (last_ && last_.type === 'leftDown') return 'upLeft'
    if (last_ && last_.type === 'upLeft') return 'rightUp'
    if (last_ && last_.type === 'rightUp') return 'downRight'
  ...
  isDead () {
    ...
    if (this.roads.length >= 2) {
      for (let i = 0, l = this.roads.length - 1; i < l; i++) {
        ...
        if (first.type === 'leftDown') console.log('FIIIIIIIIIIIIIRSSSSSSSSSSSSSSSTTTTTTTTTTT', first.type, second.type)
