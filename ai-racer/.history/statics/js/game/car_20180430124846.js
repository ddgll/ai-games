...
function Car (x, y, brain, isBest, isTest) {
  ...
  this.seight = DEFAULT_ROAD_WIDTH * 1.5
  ...
  this.sense = function (circuit) {
    ...
    for (let i = 0, l = this.angles.length, a, x, y, n, obs; i < l; i++) {
      ...
      d = this.seight
      n = (this.seight - this.r) / 2
      obs = null
      while (n > SEIGHT_PAS && d <= this.seight && d > this.r) {
        x = this.position.x + d * cos(a)
        y = this.position.y + d * sin(a)
        if (circuit.checkPoint(x, y, this.currentIndex)) {
          d += n
        } else {
          obs = this.setObstacle(obs, d, a)
          d -= n
        }
        n = n / 2
      }
  ...
  this.senseV2 = function (circuit) {
    ...
    for (let i = 0, l = this.angles.length, a, x, y, n, obs; i < l; i++) {
      ...
      x = this.position.x + d * cos(a)
      y = this.position.y + d * sin(a)
      obs = circuit.checkLine(this.position.x, this.position.y, x, y, this.currentIndex, this.seight, a)
      if (obs) {
        obstacles.push(obs)
      } else {
        obstacles.push(null)
  ...
  this.think = function (circuit) {
    if (this.killed) return
    this.obstacles = this.sensev2(circuit)
  ...
  this.update = function (circuit) {
    ...
    if (this.test && circuit) this.obstacles = this.senseV2(circuit)
  }
}
