  ...
  this.sense = function (circuit) {
    ...
    for (let i = 0, l = this.sensors.length, s, x, y; i < l; i++) {
      ...
      while (!circuit.checkPoint(x, y) && count < 1000) {
        ...
        if (s.v.x > this.x) {
          x--
          y = this.fx(this.x, this.y, s.v.x, s.y, x)
          if (x < this.x) count += 2000
        } else if (s.v.x < this.x) {
          x++
          y = this.fx(this.x, this.y, s.v.x, s.y, x)
          if (x > this.x) count += 2000
        } else if(s.v.y > this.y) {
          y--
          x = this.fy(this.x, this.y, s.v.x, s.v.y, y)
          if (y < this.y) count += 2000
        } else if(s.v.y < this.y) {
          y++
          x = this.fy(this.x, this.y, s.v.x, s.v.y, y)
          if (y > this.y) count += 2000
        } else {
          console.log('CHELOU', x, y, s, count)
          count += 2000
        }
        console.log('PUSH OOO', count, x, y)
      }
      obstacles.push({ x, y })
    }
    return obstacles
