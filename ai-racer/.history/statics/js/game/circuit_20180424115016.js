class Circuit {
  constructor (x, y, size, dadBrain, momBrain, best) {
    ...
    if (dadBrain) {
      ...
      this.brain = new neataptic.architect.Random(16, 4, 12)
    }
  }
  ...
  calculateFitness () {
    const last = this.getLastRoad()
    console.log(this.roads.length, distance(this.x, this.y, last.x, last.y), this.roads.length * 1000 / distance(this.x, this.y, last.x, last.y))
    return this.roads.length * 1000 / distance(this.x, this.y, last.x, last.y)
  }
