...
class Circuit {
  ...
  checkPoint (x, y, index) {
    ...
    if (next === this.roads.length) next = 0
    if (this.roads[previous].contains(x, y)) return true
    if (this.roads[index].contains(x, y)) return true
    if (this.roads[next].contains(x, y)) return true
    if (this.roads[next + 1].contains(x, y)) return true
  ...
  checkCar (car) {
    ...
    if (found) {
      ...
      car.checkDistance(found, index, dist)
    }
  }
