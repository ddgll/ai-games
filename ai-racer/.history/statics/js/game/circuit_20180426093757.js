...
class Circuit {
  ...
  calculateFitness (maxscore, minscore) {
    let score = 0
    let last
    for (let i = 0, l = this.size, r; i < l; i++) {
      last = this.roads[i]
      if (this.errors(r)) {
        this.dead = true
        score /= 10
    ...
    if (!this.dead && this.isClosed()) score += score * 100
    const dist = distance(this.x, this.y, last.x, last.y)
    score *= 1 / dist
    this.fitness = this.score - dist
    console.log('fff', this.score, this.fitness)
    return this.fitness
  }
  ...
  errors (road) {
    for (let i = 1, l = this.size - 1, r; i < l; i++) {
      ...
      // Intersect other
      if (r.id !== road.id && r.intersects(road.xFin, road.yFin)) return true
    }
    return false
  }
