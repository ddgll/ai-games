...
class Circuit {
  constructor (x, y, size, dna_, isBest) {
    this.id = uuid()
    this.x = x
    this.y = y
    this.dead = false
    ...
    this.roads = []
    this.dna = dna_ || this.createRandom()
    this.setBounds()
  }
  ...
  createRandom () {
    ...
    for (let i = 0, l = this.size; i < l; i++) {
      this.dna.push(Math.floor(random(3)))
    }
    console.log('createRandom', this.id, this.dna)
  }

  roadsFromDna () {
    console.log(this.id, this.dna)
    for (let i = 0, l = this.dna.length; i < l; i++) {
      this.addRoad(this.dna[i])
    }
  }
