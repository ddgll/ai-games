class Road {
  ...
  collideLine(x, y, x1, x2, angle) {
    if (this.arc !== false) {
      return lineCircleCollide(x, y, )
    } else if (this.rect !== false) {
...
function lineCircleCollide(x1, y1, x2, y2, b, xCenter, yCenter, radius, nearest) {
  ...
  //check collision
  return pointCircleCollision(nearest, circle, radius) && pLen2 <= dLen2 && (px * dx + py * dy) >= 0
}
