...
class Circuit {
  ...
  setBounds () {
    ...
    const mid = size / 2
    this.bounds = {
      xmin: this.x - DEFAULT_ROAD_LENGTH,
      ymin: this.y - DEFAULT_ROAD_LENGTH,
      xmax: this.x + mid + DEFAULT_ROAD_LENGTH,
      ymax: this.y + mid + DEFAULT_ROAD_LENGTH,
      width: size,
      height: size
    }
  }
  ...
  update () {
    ...
    if (this.roads.length < this.size) {
      if (this.isFake) {
        ...
        // Hors champ
        if (newRoad.xFin < this.bounds.xmin || newRoad.xFin > this.bounds.xmax || newRoad.yFin < this.bounds.ymin || newRoad.yFin > this.bounds.ymax) {
