function Car (x, y, brain, isBest, isTest) {
  ...
  this.setSensors = function () {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      this.sensors.push({ pos: createVector(xs, ys), vel: p5.Vector.fromAngle(i*QUARTER_PI) })
    }
  }
  ...
  this.draw = function () {
    ...
    for (let i = 0, l = this.sensors.length, s; i < l; i++) {
      s = this.sensors[i]
      stroke(0, 255, 0)
      line(this.position.x, this.position.y, s.x, s.pos.y)
    }
    for (let i = 0, l = this.obstacles.length, s; i < l; i++) {
      s = this.obstacles[i]
      stroke(0, 255, 0)
      ellipse(s.pos.x, s.pos.y, 5, 5)
    }
    pop()
  }
  ...
  this.sense = function (circuit) {
    ...
    for (let i = 0, l = this.sensors.length, s, x, y; i < l; i++) {
      ...
      while (!circuit.checkPoint(x, y)) {
        if (s.pos.x > this.x) {
          x--
          y = this.fx(this.x, this.y, s.pos.x, s.pos.y, x)
          if (x < this.x) break;
        } else if (s.pos.x < this.x) {
          x++
          y = this.fx(this.x, this.y, s.pos.x, s.pos.y, x)
          if (x > this.x) break;
        } else if(s.pos.y > this.y) {
          y--
          x = this.fy(this.x, this.y, s.pos.x, s.pos.y, y)
          if (y < this.y) break;
        } else if(s.pos.y < this.y) {
          y++
          x = this.fy(this.x, this.y, s.pos.x, s.pos.y, y)
      ...
      obstacles.push({ pos: createVector(x, y), vel: s.vel })
    }
    return obstacles
  }
