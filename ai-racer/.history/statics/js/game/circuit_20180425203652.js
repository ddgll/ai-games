...
class Circuit {
  ...
  setBounds () {
    const size = this.size * DEFAULT_ROAD_LENGTH
    const mid = size / 2
    this.bounds = {
      xmin: this.x - DEFAULT_ROAD_LENGTH * 1.5,
      ymin: this.y - DEFAULT_ROAD_LENGTH * 1.5,
      xmax: this.x + mid + DEFAULT_ROAD_LENGTH * 0.5,
      ymax: this.y + mid + DEFAULT_ROAD_LENGTH * 0.5,
      width: size,
      height: size
    }
  }
  ...
  getNextType (last_) {
    ...
    return this.getNextTypeFromDirection(label, last.type)
  }

  getNextTypeFromDirection (dir, type) {
    if (!type) type = 'rightDown'
    const reg = /^([^A-Z]+)(.*$)/
    const debut = type.replace(reg, '$1')
    const fin = type.replace(reg, '$2').toLowerCase()
    if (debut !== 'left' && debut !== 'right' && fin !== 'left' && fin !== 'right' && dir === 'none') return type || 'rightDown'
    const ntype = this.getInverse(fin)
    switch (ntype) {
      case 'up':
        if (dir === 'none') return 'upDown'
        if (dir === 'right') return 'upLeft'
        if (dir === 'left') return 'upRight'
        break;
      case 'down':
        if (dir === 'none') return 'downUp'
        if (dir === 'right') return 'downRight'
        if (dir === 'left') return 'downLeft'
        break;
      case 'left':
        if (dir === 'none') return 'leftRight'
        if (dir === 'right') return 'leftDown'
        if (dir === 'left') return 'leftUp'
        break;
      case 'right':
        if (dir === 'none') return 'rightLeft'
        if (dir === 'right') return 'rightUp'
        if (dir === 'left') return 'rightDown'
        break;
    }
  }
