function Car (x, y, brain, isBest, isTest) {
  ...
  this.sense = function (circuit) {
    ...
    for (let i = 0, a, x, y, n; i < 8; i++) {
      ...
      v = createVector(x, y)
      n.mult(pas)
      console.log('BEGIN DISTANCE', x, y, this.position.x, this.position.y, n.x, n.y, v.x, v.y)
      while (pas < 1) {
        n = v.copy()
        n.mult(pas)
        d = n.dist(this.position)
        console.log('DISTANCE', d, v.x, v.y, this.position.x, this.position.y, circuit.checkPoint(v.x, v.y))
        if (!circuit.checkPoint(n.x, n.y)) {
          obstacles.push({ v: v, d })
          pas = 1000
        }
        v.div(pas)
        pas+=pas
