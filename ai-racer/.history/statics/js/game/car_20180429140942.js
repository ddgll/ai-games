...
function Car (x, y, brain, isBest, isTest) {
  ...
  this.draw = function () {
    ...
    if (this.test || this.best) {
      for (let i = 0, l = this.angles.length, as, sv; i < l; i++) {
        a = this.angles[i] + this.headings
        stroke(0, 255, 0)
        sv = createVector(this.seight * cos(a), this.seight * sin(a))
        line(0, 0, sv.x, sv.y)
      }
    }
    pop ()
  ...
  this.sense = function (circuit) {
    ...
    for (let i = 0, l = this.angles.length, a, x, y, n, obs; i < l; i++) {
      a = this.angles[i] // (-HALF_PI + i * QUARTER_PI + this.heading)
      d = this.seight
  ...
  this.checkDistance = function (road, index, distance) {
