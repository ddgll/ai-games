...
class Circuit {
  ...
  setBounds () {
    const lengthX = Math.round(Math.sqrt(this.size)) + 2
    const lengthY = (lengthX - 2) * (lengthX - 2) < this.size ? lengthX + 1 : lengthX
  ...
  offBounds (road) {
    if (road.arc) {
      return (road.xCollide - (DEFAULT_ROAD_LENGTH / 2) < this.bounds.xmin || 
        road.xCollide + (DEFAULT_ROAD_LENGTH / 2) > this.bounds.xmax || 
        road.yCollide - (DEFAULT_ROAD_LENGTH / 2) < this.bounds.ymin || 
        road.yCollide + (DEFAULT_ROAD_LENGTH / 2) > this.bounds.ymax)
    } else {
      return (road.xFin - (DEFAULT_ROAD_LENGTH / 2) < this.bounds.xmin || 
        road.xFin + (DEFAULT_ROAD_LENGTH / 2) > this.bounds.xmax || 
        road.yFin - (DEFAULT_ROAD_LENGTH / 2) < this.bounds.ymin || 
        road.yFin + (DEFAULT_ROAD_LENGTH / 2) > this.bounds.ymax)
    }
  }
