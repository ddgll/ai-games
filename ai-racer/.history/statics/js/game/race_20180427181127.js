...
function draw () {
  ...
  for (let i = 0; i < maxc; i++) {
    if (circuit) {
      if (testCar) testCar.update(circuit)
      if (cars) {
        cars.forEach(c => {
          c.think(circuit)
          c.update(circuit)
        })
        const reste = cars.map(c => !c.killed).length
        if (reste === 0) {
  ...
  if (neat) {
    log.elt.innerHTML = '<small>FPS multiplieur: ' + maxc + '</small> Frames: ' +  ' gen: ' + neat.generation + ' Cars:' + cars.length + '<br>'
  }
...
function mouseMoved() {
  if (circuit) {
    circuit.checkCar({ position: { x: mouseX, y: mouseY }})
  }
}
