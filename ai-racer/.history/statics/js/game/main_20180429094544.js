...
function setup () {
  ...
  roads = [
    ...
    new Road(200, 110, 'leftDown'),
    new Road(300, 110, 'rightUp'),
    new Road(40, 110, 'rightDown'),

    new Road(100, 160, 'upLeft'),
    new Road(200, 160, 'downLeft'),
    new Road(300, 160, 'upRight'),
    new Road(400, 160, 'downRight')
  ]
}
...
function draw () {
  ...
  if (winners && winners.length) {
    ...
    winners.forEach(w => {
      ...
      w.draw()
      db.circuits.put({
        id: w.roads.map(r => r.number).join('-'),
        size: w.size,
        dna: w.dna
      })
