    ...
    if (this.roads.length >= 2) {
      ...
      for (let i = 0, l = this.roads.length - 1; i < l; i++) {
        ...
        const errors = this.roads.filter(r => {
          // Exists
          if (r.id !== first.id && ((r.x === first.x && r.y === first.y) || (r.xFin === first.xFin && r.yFin === first.yFin))) {
            return true
          }
          // Reach
          // if (r.id !== begin.id && r.xFin === begin.x && r.yFin === begin.y) {
          //   return true
          // }
          // Block finish
          // if (r.id !== last.id && r.id !== begin.id && r.intersects(last.xFin, last.yFin)) {
          //   return true
          // }
          return false
        })
