  ...
  isDead (last) {
    ...
    if (this.roads.length >= 2) {
      ...
      for (let i = 0, l = this.roads.length - 1; i < l; i++) {
        ...
        const exists = this.roads.filter(r => r.id !== first.id && r.x === first.x && r.y === first.y)
        if (exists.length > 0) {
          // console.log('Road exists')
          return true
        }
        if (last && last.xFin === begin.x && last.yFin === begin.y && this.roads.length !== this.size) {
  ...
  update () {
    ...
    if (this.roads.length < this.size) {
      if (this.isFake) {
        ...
        const newRoad = new Road(last.x, last.y, this.getNextType(last))
