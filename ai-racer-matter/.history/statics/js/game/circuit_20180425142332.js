  ...
  isDead (last) {
    ...
    if (this.roads.length >= 2) {
      ...
      for (let i = 0, l = this.roads.length - 1; i < l; i++) {
        ...
        const exists = this.roads.filter(r => r.id !== first.id && ((r.x === first.x && r.y === first.y) || (r.xFin === first.xFin && r.yFin === first.yFin)))
        if (exists.length > 0) {
          // console.log('Road exists')
          return true
        }
        const reach = this.roads.filter(r => r.id !== begin.id && r.xFin === begin.x && r.yFin === begin.y)
        if (reach.length > 0 && this.roads.length < this.size) {
          // console.log('Reach end before end')
          return true
        }
      }
    }
