class Circuit {
  ...
  oneHotEncode(type){
    const zeros = Array.apply(null, Array(this.types.length)).map(Number.prototype.valueOf, 0);
  ...
  oneHotDecode(zeros){
    const index = this.types.findIndex(zeros);
  ...
  getNextType (last) {
    ...
    console.log('NEXT LABEL', label, outputs, inputs)
    return label
