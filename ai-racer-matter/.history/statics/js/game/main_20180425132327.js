...
function draw () {
  ...
  if (best && neat.generation) {
    log.elt.innerHTML += '<h1 style="margin: 4px 0">Gen:' + neat.generation + '- best score:' + Math.round(bestScore) + '- best size:' + bestSize + ' best gen: ' + bestNumGen + '</h1>'
    log.elt.innerHTML += 'best fitness:' + bestFitness
    log.elt.innerHTML += '<br>best path: ' + best.serialize()
    log.elt.innerHTML += '<br><small>x' + maxc + ' f' + frameR + '</small>'
  }
