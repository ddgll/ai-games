function Car (x, y, brain, isBest, isTest) {
  ...
  this.draw = function () {
    ...
    pop()
    for (let i = 0, l = this.obstacles.length, x, y; i < l; i++) {
      x = this.position.x + this.obstacles[i] * cos(this.heading + PI / 2)
      y = this.position.y + this.obstacles[i] * sin(this.heading + PI / 2)
      stroke(255, 0, 0)
      ellipse(x, y, 5, 5)
    }
  }
