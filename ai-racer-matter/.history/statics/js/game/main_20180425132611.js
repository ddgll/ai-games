...
function draw () {
  ...
  if (best && neat.generation) {
    str.concat('<h1 style="margin: 4px 0">BEST - score:' + Math.round(bestScore) + ' - size:' + bestSize + ' - gen: ' + bestNumGen + '</h1>')
    str.concat('best fitness:' + bestFitness)
    str.concat('<br>best path: ' + best.serialize())
    str.concat('Gen:' + neat.generation)
    str.concat('<br><small>x' + maxc + ' f' + frameR + '</small>')
  }
  ...
  for (let i = 0; i < maxc; i++) {
    ...
    if (reste === 0) {
      ...
      console.log('RESTE', reste, circuits.filter(c => !c.dead))
    }
  }
  background(0)
