  ...
  isDead () {
    ...
    if (this.roads.length >= 2) {
      for (let i = 0, l = this.roads.length - 1; i < l; i++) {
        ...
        if (findFirst === debutSecond) return true
        if (this.isStraight(first) && this.isStraight(second) && first.type !== second.type) return true




        if (first.xFin !== second.x && first.yFin !== second.y) return true
        if (debutSecond === finFirst) return true
