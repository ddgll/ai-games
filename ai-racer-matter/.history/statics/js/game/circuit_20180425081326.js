...
class Circuit {
  ...
  calculateFitness (maxscore, minscore) {
    ...
    this.fitness = dist === 0 ? 0 : this.roads.length / dist
    console.log('fff', this.normalizedScore, this.fitness, this.score, maxscore, minscore)
  ...
  update () {
    ...
    if (!this.dead) {
      ...
      if (last) {
        const dist = distance(this.x, this.y, last.x, last.y)
        if (this.lastDist !== null && dist < this.lastDist) {
          this.score += 2
        }
        this.lastDist = dist
        if (this.roads.length > this.size / 2 && dist > this.lastDist) {
          this.score -= 5
        }
      }
      if (this.roads.length === this.size) {
