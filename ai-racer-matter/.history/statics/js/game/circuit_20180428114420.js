...
class Circuit {
  ...
  click (event) {
    if (!event || !event.source || !event.source.body) return
    const id = event.source.body.id
    const road = this.roads.find(r => r.body.id === id)
    if (road) road.setIsWall(false)
  }

  createWalls () {
    let row = 0, y = this.y
    let rWidth = Math.sqrt(3) * this.roadsSize
    let rHeight = rWidth - 5 // this.roadsSize * 2
    console.log('ROAD w:', rWidth, 'h:', rHeight)
    while (y + rHeight <= this.height) {
      let col = 0, offset = this.x
      if (row % 2 === 1) {
        offset = this.x + rWidth / 2
        col = 1
      }
      let x = offset
      while ( x + rWidth <= this.width) {
        console.log('ROAD', x, y)
    ...
    return Body.create({ parts: bodies, isStatic: true })
  }
