...
function init () {
  ...
  new p5(function (p) {
    p.setup = function () {
      ...
      p.createDiv('<h1>' + numGeneration + ' BEST Score: ' + bsaved + '<h1/>')
    }
  }, 'master')
...
function createGames(brain_, brains) {
  ...
  new p5(sketch(brain_, 'game', bestDeath, pipes, games, true), 'game')
  for (let i = 0; i < POPULATION; i++) {
    let brain = brains && brains.length ? createBaby(brains) : brain_
    new p5(sketch(brain, 'game' + i, childDeath, pipes, games, false), 'game' + i)
  }
}
...
function createBaby (parents) {
  ...
  return neataptic.Network.fromJSON(dadBrain) // neataptic.Network.crossOver(neataptic.Network.fromJSON(dadBrain), neataptic.Network.fromJSON(momBrain))
}
