...
function createGames(brain_, brains) {
  ...
  for (let i = 0; i < POPULATION; i++) {
    let brain = brains && brains.length ? createBaby(brains, brain_) : brain_
...
function createBaby (parents, brain_) {
  // const rd = Math.random()
  const dadBrain = pickOneGenome(parents).brain
  // if (rd > 0.33 && rd < 0.66) return neataptic.Network.fromJSON(dadBrain)
  // if (rd < 0.33) return brain_
