...
function draw () {
  ...
  if (JSON.stringify(resized.shape) === `[${IMAGE_SIZE},${IMAGE_SIZE},4]`) {
    ...
    // memory.push(resized.reshape(IMAGE_SIZE, IMAGE_SIZE).flatten())
    console.log('WRONG Shape', JSON.stringify(resized.shape), img.shape)
  }
...
function addFrames () {
  ...
  memory.forEach(frame => {
    ...
    v.selection.data.forEach((pixel, index) => {
      ...
      if (index % IMAGE_SIZE < left) v.selection.data[index] = BACKGROUND
    })
