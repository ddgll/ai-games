...
function draw () {
  const img = nj.images.read(canvas.canvas)
  const resized = nj.images.resize(img, 28, 28)
  memory.push(resized)
  if (memory.length > 10) {
...
function addFrames () {
  let proceed
  for (let frame of memory) {
    if (!proceed) {
      proceed = frame
    } else {
      proceed = proceed.add(frame, false)
    }
  }
