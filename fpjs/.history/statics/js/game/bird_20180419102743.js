function Bird (dadBrain, momBrain, best) {
  ...
  this.reduce = function (a) {
    ...
    for(let i = 0; i < a.length; i += n_averaged_elements) {
      ...
      averaged_array = averaged_array.concat(np.mean(a.slice(slice_from_index, slice_to_index)))
    }
  }
  ...
  this.think = function (vision) {
    ...
    const long = nj.array(vision.tolist().concat(this.positions.reduce((a, b) => a.concat(b))))
    const window_sz = 3
    const reshaped = nj.reshape(long, 6).mean(1)
