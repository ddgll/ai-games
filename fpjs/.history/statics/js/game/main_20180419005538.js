...
function draw () {
  //if (counter > 3) {
    ...
    if (JSON.stringify(resized.shape) === `[${IMAGE_SIZE},${IMAGE_SIZE},4]`) memory.push(resized.flatten().toarray())
...
function addFrames () {
  vision = nj.zeros([IMAGE_SIZE, IMAGE_SIZE, 4]).flatten().toarray()
  vision.assign(BACKGROUND, false)
  memory.forEach(frame => {
    frame = frame.flatten()
    vision.forEach((pixel, index) => {
      if (pixel === BACKGROUND) {
        pixel = frame[index]
      }
    })
