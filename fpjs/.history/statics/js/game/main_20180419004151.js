...
function draw () {
  //if (counter > 3) {
    ...
    if (JSON.stringify(resized.shape) !== '[128,128,4]') memory.push(resized)
...
function addFrames () {
  ...
  memory.forEach(frame => {
    if (JSON.stringify(frame.shape) !== '[128,128,4]') return
    frame = frame.reshape(IMAGE_SIZE,IMAGE_SIZE,4)
    if (!vision) {
      vision = frame
    } else {
