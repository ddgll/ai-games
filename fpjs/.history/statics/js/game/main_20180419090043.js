...
function draw () {
  ...
  if (JSON.stringify(resized.shape) === `[${IMAGE_SIZE},${IMAGE_SIZE},4]`) {
    console.log('resized', resized, resized.flatten())
    console.log(resized.reshape(IMAGE_SIZE, IMAGE_SIZE, 4))
...
function addFrames () {
  ...
  memory.forEach(frame => {
    if (!vision) vision = frame
    vision.selection.data.forEach((pixel, index) => {
      if (pixel === BACKGROUND) {
        vision.selection.data[index] = frame.selection.data[index]
      }
    })
    // vision = vision ? vision.add(frame, false) : frame
  })
  if (vision) {
    console.log('SET VISION', nj.uint8(vision).reshape(IMAGE_SIZE, IMAGE_SIZE, 4))
    var ca = document.getElementById('resized')
    nj.images.save(nj.uint8(vision).reshape(IMAGE_SIZE, IMAGE_SIZE, 4), ca)
  }
}
