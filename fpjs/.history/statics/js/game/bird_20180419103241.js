function Bird (dadBrain, momBrain, best) {
  ...
  while (this.reduce(test).length > 20) this.reshapeGroup++
  ...
  this.reduce = function (a) {
    n_averaged_elements = this.reshapeGroup
    averaged_array = []
  ...
  this.think = function (vision) {
    ...
    const long = vision.tolist().concat(this.positions.reduce((a, b) => a.concat(b)))
