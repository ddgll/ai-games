...
function init () {
  ...
  try {
    const brains = JSON.parse(localStorage.getItems('brains'))
    if (brains && brains.length) {
      createGames(brain, true, brains)
    }
  } catch (e) { }
  createGames(brain)
}
...
function createGames(brain_, next, brains) {
  let brain = next ? createBaby(brains) : brain_
  container.innerHTML = ''
