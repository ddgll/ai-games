...
function setup () {
  ...
  if (bshowb === 'true') showb = true
  if (bshowc === 'true') showclones = true
  ...
  button.mousePressed(() => {
    ...
    localStorage.setItem('bestshowc', showc)
  })
}
...
function draw () {
  ...
  log.elt.innerHTML = counter + ' h(' + hard + ') : ' + ' sh(' + shard + ') : ' + birds.length + '<br>' + 
    '<h1>BEST: ' + toDate(bfrs) + ' <small>FPS multiplieur: ' + maxc + '</small><h1>'
  ...
  if (showclones) {
    ...
    birds.forEach(b => (b.first) ? b.show() : null)
  }
}
