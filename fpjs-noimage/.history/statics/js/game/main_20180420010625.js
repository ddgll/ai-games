...
function draw () {
  ...
  for (let i = 0; i < slider.value(); i++) {
    ...
    log.elt.innerHTML = counter + ' h(' + hard + ') : ' + birds.length + ''
    if (counter > 50000) log.elt.innerHTML += ' sh'
    ...
    if (birds.length < POPULATION*0.1 && bestScore > 100000) {
      birds.forEach(bird => {
        bird.killed = true
      })
    }
