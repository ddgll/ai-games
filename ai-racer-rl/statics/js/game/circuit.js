...
class Circuit {
  ...
  getOptimal(car, heading_ = null) {
    if (car.road) {
      const heading = heading_ !== null ? heading_ : car.heading
      const plus = car.road.isStraight() ? -HALF_PI : 0
      const vCar = p5.Vector.fromAngle(heading - plus)
      const vRoad = createVector(car.road.yFin - car.road.y, car.road.xFin - car.road.x)
      const dot = vRoad.angleBetween(vCar)
      const roadAngle = (Math.atan2(car.road.yFin - car.road.y, car.road.xFin - car.road.x) + TWO_PI) % TWO_PI
      car.sens = roadAngle - heading > 0 ? 1 : 0
      if (car.road.needMinusDot()) {
        return 1 - normalize(dot, 0, PI)
      } else {
        return normalize(dot, 0, PI)
      }
    }
    return 1
  }
